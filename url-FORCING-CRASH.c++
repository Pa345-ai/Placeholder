...
void testFinding1DirectOOB() {
  // Direct test of the vulnerable pattern WITHOUT normalizePathEncoding wrapper
  // This forces the exact OOB dereference that should trigger ASAN
  
  // Test case 1: "/foo%2"
  {
    kj::String str = kj::str("/foo%2");
    std::string_view input(str.begin(), str.end());
    fprintf(stderr, "Test 1: input='%.*s' length=%zu\n", (int)input.length(), input.data(), input.length());
    
    size_t pos = input.find("%2", 0);
    if (pos != std::string_view::npos) {
      fprintf(stderr, "Found %%2 at pos %zu, accessing input[%zu]\n", pos, pos + 2);
      fflush(stderr);
      
      // THIS IS THE VULNERABLE DEREFERENCE
      // pos + 2 == input.length(), so this is OOB
      volatile char c = input[pos + 2];  // ASAN MUST CATCH THIS
  ...
  // Test case 2: "%2"
  {
    kj::String str = kj::str("%2");
    std::string_view input(str.begin(), str.end());
    fprintf(stderr, "Test 2: input='%.*s' length=%zu\n", (int)input.length(), input.data(), input.length());
  ...
  // Test case 3: "foo%2"
  {
    kj::String str = kj::str("foo%2");
    std::string_view input(str.begin(), str.end());
    fprintf(stderr, "Test 3: input='%.*s' length=%zu\n", (int)input.length(), input.data(), input.length());
