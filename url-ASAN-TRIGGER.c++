...
kj::Array<const char> normalizePathEncoding(kj::ArrayPtr<const char> pathname) {
  ...
  static constexpr auto findNext = [](std::string_view input) -> kj::Maybe<size_t> {
    ...
    if (pos != std::string_view::npos) {
      fprintf(stderr, "DEBUG: Found %%2 at position %zu, input length %zu\n", pos, input.length());
      fflush(stderr);
      
      // CHECK: Is pos + 2 actually in bounds?
      if (pos + 2 >= input.length()) {
        fprintf(stderr, "DEBUG: OOB ACCESS WOULD OCCUR: pos+2=%zu >= length=%zu\n", pos + 2, input.length());
        fflush(stderr);
      }
      
      if (input[pos + 2] == 'f' || input[pos + 2] == 'F') {
        fprintf(stderr, "DEBUG: Matched f/F at position %zu\n", pos + 2);
        fflush(stderr);
        return pos;
      }
    }
...
void testFinding1DirectOOB() {
  // CRITICAL: Use exact-sized heap arrays WITHOUT null terminator
  // This places the ASAN poisoned redzone directly after the allocation
  // Forcing ASAN to catch the OOB read
  
  fprintf(stderr, "=== Finding #1 OOB Trigger Test ===\n");
  fflush(stderr);
  
  // Test case 1: Exactly 6 bytes - "/foo%2"
  // Allocate EXACTLY 6 bytes with no padding or null terminator
  {
    fprintf(stderr, "Test 1: Exact 6-byte allocation\n");
    fflush(stderr);
    auto path1 = kj::heapArray<char>({'/', 'f', 'o', 'o', '%', '2'});
    
    fprintf(stderr, "Calling normalizePathEncoding with exact 6-byte array...\n");
    fflush(stderr);
    
    // This will access input[pos + 2] = input[6]
    // Which lands directly in the ASAN poisoned redzone after the allocation
    auto result1 = normalizePathEncoding(path1.asPtr());
    
    fprintf(stderr, "Test 1 completed (should have crashed)\n");
    fflush(stderr);
  }
  
  // Test case 2: Exactly 2 bytes - "%2"
  {
    fprintf(stderr, "Test 2: Exact 2-byte allocation\n");
    fflush(stderr);
    auto path2 = kj::heapArray<char>({'%', '2'});
    
    fprintf(stderr, "Calling normalizePathEncoding with exact 2-byte array...\n");
    fflush(stderr);
    
    // This will access input[pos + 2] = input[2]
    // Which lands directly in the ASAN poisoned redzone after the allocation
    auto result2 = normalizePathEncoding(path2.asPtr());
    
    fprintf(stderr, "Test 2 completed (should have crashed)\n");
    fflush(stderr);
  }
  
  // Test case 3: Exactly 5 bytes - "foo%2"
  {
    fprintf(stderr, "Test 3: Exact 5-byte allocation\n");
    fflush(stderr);
    auto path3 = kj::heapArray<char>({'f', 'o', 'o', '%', '2'});
    
    fprintf(stderr, "Calling normalizePathEncoding with exact 5-byte array...\n");
    fflush(stderr);
    
    // This will access input[pos + 2] = input[5]
    // Which lands directly in the ASAN poisoned redzone after the allocation
    auto result3 = normalizePathEncoding(path3.asPtr());
    
    fprintf(stderr, "Test 3 completed (should have crashed)\n");
    fflush(stderr);
  }
}
